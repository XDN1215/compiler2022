...
bool begin_semanalysis(TreeNode *root){
    HashTable_Initial(st);stack_sn = Stack_New(100);strdef = Stack_New(100);strdef->top = -1;//初始化数据结构
...
{ //分类讨论，变量类型-BASIC/ARRAY/STRUCTVAR/
    if (tn != NULL){
        ...
        {
            Child1->inh = tn->inh;
            switch (tn->inh->type->kind) // Specifier只可能为BASIC或者STRUCTURE
            {
            case BASIC:
                strcpy(Child1->syn->fieldname, Child1->strval);
    ...
    if (tn != NULL){
        ...
        if (tn->isfundec == 0) //函数定义
        {
            if (tn->childtype == 1) // ID LP VarList RP
            {
                ...
                tn->syn->type->uval.function.argn = funparan + 1;
                funparan = -1;
                tn->syn->type->uval.function.args = Child3->syn;
            ...
            else if (tn->childtype == 2) // ID LP RP
            {
                ...
                tn->syn->type->kind = FUNCTIONDEF;
                tn->syn->type->uval.function.return_type = tn->inh->type;
                tn->syn->type->uval.function.argn = 0;
                tn->syn->type->uval.function.args = NULL;
...
bool Analyse_CompSt(TreeNode *tn){
    if (tn != NULL){
        ...
        if (tn->childtype == 1) // LC DefList StmtList RC
        {
            ...
            {
                FieldListp args = tn->inh->type->uval.function.args;
                while (args != NULL)
                {
                    SymbolNodep newsn = SymbolNode_Set(args->fieldname, LOCAL, stack_sn->top, args->type);
                    HashTable_Insert(st, newsn);
                    Stack_Link(stack_sn, newsn);
                    args = args->tail;
                }
...
if (tn != NULL)
    {
        ...
        case 12: // ID LP Args RP
        {
            ...
            if (p != NULL){
                ...
                if (isright2){
                    if (TypeCheck_FieldList(Child3->syn, p->type->uval.function.args) == false){                       
    ...
    if (type1->kind == type2->kind && type1->kind > 0)
    {
        ...
        case FUNCTIONDEC:
            if (TypeCheck_type(type1->uval.function.return_type, type2->uval.function.return_type))
                if (type1->uval.function.argn == type2->uval.function.argn)
                    if (TypeCheck_FieldList(type1->uval.function.args, type2->uval.function.args))
                        return true;
            break;
        case FUNCTIONDEF:
            if (TypeCheck_type(type1->uval.function.return_type, type2->uval.function.return_type))
